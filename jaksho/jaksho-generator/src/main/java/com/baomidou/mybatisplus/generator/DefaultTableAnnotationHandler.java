    ...
    public List<ClassAnnotationAttributes> handle(TableInfo tableInfo, Entity entity) {
        ...
        if (tableInfo.isConvert()) {
            ...
            if (anyCustomField.isEmpty()) {
                ...
                displayName = String.format("@TableName(\"%s%s\")", schemaName, tableInfo.getName());
            } else {
                // @TableName(value = "${schemaName}${table.name}", autoResultMap = true)
                displayName = String.format("@TableName(value = \"%s%s\", autoResultMap = true)", schemaName, tableInfo.getName());
            }
