...
public class DefaultTableFieldAnnotationHandler implements ITableFieldAnnotationHandler {
    ...
    public List<AnnotationAttributes> handle(TableInfo tableInfo, TableField tableField) {
        ...
        if (tableField.isKeyFlag()) {
            ...
            if (StringUtils.isNotBlank(fill)) {
                ...
                if (tableField.isConvert()) {
                    String displayName = getDisplayName(tableField);
                    annotationAttributesList.add(new AnnotationAttributes(displayName, com.baomidou.mybatisplus.annotation.TableField.class.getName()));
                }
            }
    ...
    private static String getDisplayName(TableField tableField) {
        ...
        if (tableField.getColumnType() instanceof CustomColumnType) {
            // feature: 自定义字段处理
            // @TableField(value = "${field.annotationColumnName}", typeHandler = com.baomidou.mybatisplus.extension.handlers.Fastjson2TypeHandler.class)
            displayName = String.format("@TableField(value = \"%s\", typeHandler = %s.class)", tableField.getAnnotationColumnName(), Fastjson2TypeHandler.class.getName());
        } else {
            //@TableField("${field.annotationColumnName}")
            displayName = String.format("@TableField(\"%s\")", tableField.getAnnotationColumnName());
        }
        return displayName;
    }
